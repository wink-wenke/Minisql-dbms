    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="flex items-center justify-between px-4 py-2 border-b border-border bg-bg-surface">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-semibold">模糊测试</span>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            <label className="text-xs text-text-secondary">迭代次数：</label>
            <input
              type="number"
              value={count}
      ...
      <div className="flex gap-4 px-4 py-3 border-b border-border bg-bg-surface">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-mono text-text-secondary">正常：<span className="text-green">{ok}</span></span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-red" />
          <span className="text-xs font-mono text-text-secondary">报错：<span className="text-red">{errors}</span></span>
        </div>
        <div className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-yellow" />
          <span className="text-xs font-mono text-text-secondary">崩溃：<span className="text-yellow">{crashes}</span></span>
        </div>
        {entries.length > 0 && (
